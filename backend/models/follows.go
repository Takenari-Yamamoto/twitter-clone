...
type Follow struct {
	...
	FollowingID int64     `boil:"following_id" json:"following_id" toml:"following_id" yaml:"following_id"`
...
var FollowWhere = struct {
	...
	FollowingID whereHelperint64
	CreatedAt   whereHelpertime_Time
}{
	ID:          whereHelperint64{field: "`follows`.`id`"},
	FollowerID:  whereHelperint64{field: "`follows`.`follower_id`"},
	FollowingID: whereHelperint64{field: "`follows`.`following_id`"},
...
var (
	followAllColumns            = []string{"id", "follower_id", "following_id", "created_at"}
...
func FindFollow(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Follow, error) {
	...
	query := fmt.Sprintf(
		"select %s from `follows` where `id`=?", sel,
	)
...
func FollowExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `follows` where `id`=? limit 1)"
